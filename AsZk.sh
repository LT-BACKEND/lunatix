...
clear;clear;clear
echo -e "${BlueBee}╔════════════════════════════════════════════════╗${NC}"
echo -e "\033[96;1m                    LUNATIC BACKEND                  \033[0m"
echo -e "${BlueBee}╚════════════════════════════════════════════════╝${NC}"
echo ""
echo ""
...
echo -e "\e[32mloading...\e[0m"
clear
REPO="https://raw.githubusercontent.com/LT-BACKEND/lunatix/bcknd/"
start=$(date +%s)
secs_to_human() {
...
mkdir -p /var/lib/lunatic >/dev/null 2>&1
...
# // Notif Botqu
IDIQU="-1002154836378"
KEYQU="7258491552:AAG79Rd7RzqplUwmw5zc8WMdDNhd9ZH_W3w"
URLQU="https://api.telegram.org/bot$KEY/sendMessage"
...
echo -e "\e[94;1m╚════════════════════════════════════════════════╝ \e[0m"
echo -e ""
echo -e "              \e[97;1m 1. \e[92;1m Input Your Domain"
echo -e "              \e[97;1m 2. \e[92;1m Random Domain \e[0m "
echo -e ""
echo -e "\e[94;1m╚═════════════════════════════════════════════════╝ \e[0m"
echo
echo
...
<code>────────────────────</code>
<code>Thank you for ordering at-</code>
<code>    Lunatic tunneling     </code>
<code>────────────────────</code>
"'&reply_markup={"inline_keyboard":[[{"text":"ᴏʀᴅᴇʀ","url":"https://t.me/LunaticTunnel"},{"text":"Contack","url":"wa.me/6285955333616"}]]}'
...
mkdir -p /etc/lunatic/limit/trojan/shadowsocks
# // dir protocol
mkdir -p /etc/lunatic/vmess
mkdir -p /etc/lunatic/vless
mkdir -p /etc/lunatic/trojan
...
wget "http://raw.githubusercontent.com/LT-BACKEND/lunatix/bcknd/Fls/limit.sh && chmod +x limit.sh && ./limit.sh"
cd
wget -q -O /usr/bin/lock-xray-ip "${REPO}Fls/lock-xray-ip"
chmod +x /usr/bin/*
cd /usr/bin
sed -i 's/\r//' lock-xray-ip
cd
clear
...
ExecStart=/usr/bin/lock-xray-ip vmip
Restart=always
[Install]
WantedBy=multi-user.target
EOF
...
[Service]
WorkingDirectory=/root
ExecStart=/usr/bin/lock-xray-ip vlip
Restart=always
[Install]
WantedBy=multi-user.target
EOF
...
[Service]
WorkingDirectory=/root
ExecStart=/usr/bin/lock-xray-ip trip
Restart=always
[Install]
WantedBy=multi-user.target
EOF
...
[Service]
WorkingDirectory=/root
ExecStart=/usr/bin/lock-xray-ip ssh
Restart=always
[Install]
WantedBy=multi-user.target
EOF
...
[Service]
WorkingDirectory=/root
ExecStart=/usr/bin/lock-xray-ip ssip
Restart=always
[Install]
WantedBy=multi-user.target
EOF
...
welcome
EOF
cat >/etc/cron.d/sshlimit <<-END
SHELL=/bin/sh
PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin
*/1 * * * * root /usr/local/sbin/limitssh-ip
END
cat >/etc/cron.d/xrylimit <<-END
SHELL=/bin/sh
PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin
*/1 * * * * root /usr/local/sbin/limit-ip
END
cat >/etc/cron.d/quotalimit <<-END
SHELL=/bin/sh
PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin
*/1 * * * * root /usr/local/sbin/quota
END
cat >/etc/cron.d/delmullog <<-END
SHELL=/bin/sh
PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin
*/1 * * * * root /usr/local/sbin/mullog
END
cat >/etc/cron.d/autobackup <<-END
SHELL=/bin/sh
PATH=/usr/local/sbin:/usr/local/bin:/sbin:/bin:/usr/sbin:/usr/bin
*/10 * * * * root /usr/local/sbin/backupvps
END
...
dnsslow(){
wget ${REPO}Fls/slowdns.sh
chmod +x slowdns.sh && ./slowdns.sh
}
clear
function instal(){
clear
...
clear
echo -e "\e[94;1m╔═════════════════════════════════════════════════╗\e[0m"
echo -e "\e[96;1m              ----[ SUKARNA CELL ]----                    \e[0m"
echo -e "\e[94;1m╚═════════════════════════════════════════════════╝\e[0m"
echo ""
echo -e "\e[95;1m  Whatsapp : 085955333616 \e[0m"
